...
def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None, n_jobs=1, train_sizes=np.linspace(.1, 1.0, 5)):
    ...
    plt.fill_between(train_sizes, test_scores_mean,
                     test_scores_mean, alpha=0.1, color="g")
    plt.plot(train_sizes, train_scores_mean, 'v-', color="r",
...
def plot_validation_curve(estimator, title, X, y, param_name, param_range, ylim=None):
    ...
    if isinstance(param_range[0], str):
        plt.scatter(param_range, train_scores_mean, s=40, label="Training score", color="r", marker="v")
        plt.scatter(param_range, test_scores_mean, s=40, label="Cross-validation score", color="g", marker="o")
    else:
        plt.plot(param_range, train_scores_mean, 'v-', label="Training score", color="r", lw=lw)
